...
public class NewAstrologyController implements Resettable {
    ...
    private void setBufferLocation(ToggleGroup currentGroup, AstroSymbol[] searchArray, AstrologyIndex index) {
        ...
        for (int i = 0; i < searchArray.length; i++) {
            if (toggleName.equals(searchArray[i].name())) {
                ...
                i = astroSymbolBuffer.length;
            }
        }

        if (astroSymbolBuffer[0] != null && astroSymbolBuffer[1] != null && astroSymbolBuffer[2] != null)
            processBuffer();
    }
